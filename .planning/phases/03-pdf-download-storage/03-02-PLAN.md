...
<task type="auto">
  ...
  <action>
    ...
    3. **`_download_filing(filing, settings, http_client)` helper**:
       - Takes a Filing ORM object (with eagerly loaded documents), PipelineSettings, and httpx.Client
       ...
       - Calls `wait_between_requests(settings.delay_min_seconds, settings.delay_max_seconds)` between downloads (import from `cer_scraper.scraper.rate_limiter`)
    ...
    4. **`download_filings(session, settings)` public function**:
       - Takes SQLAlchemy session and PipelineSettings
    ...
    - Import `ScraperSettings` is NOT needed -- use PipelineSettings only. Rate limiter delay values come from ScraperSettings but the orchestrator gets them as parameters. Actually, looking at the rate limiter, it takes min/max as parameters. The orchestrator should accept BOTH PipelineSettings AND ScraperSettings, or just pass delay values from ScraperSettings. Per CONTEXT.md: "Reuse the existing scraper rate limiter (1-3 second delay)". So `download_filings` should accept `pipeline_settings: PipelineSettings` and `scraper_settings: ScraperSettings` as separate parameters, using `scraper_settings.delay_min_seconds` and `scraper_settings.delay_max_seconds` for the rate limiter calls.
